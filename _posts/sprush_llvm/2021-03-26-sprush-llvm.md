...
## LLVM
![](/_posts/sprush_llvm/7tY1edP.png)
...
## LLVM IR
Если коротко, LLVM IR можно охарактеризовать как типизированный трехадресный код в SSA форме. Что такое SSA я говорил ранее, а трехадресный код - это последовательность операторов вида `x := y op z`, где `x`, `y` и `z` - это имена, константы или сгенерированные компилятором временные объекты. В правую часть после равно может входить только один знак операции.
